...
-export([substmts_to_json/2, is_substmt/1, emit_sn_json/5, kw2str/1]).
...
emit(Ctx, Mods, Fd) ->
    MainMod = proplists:get_value(main_module, Ctx#yctx.options),
...
emit_mod(Mod, Fd) ->
    ...
    TopState = lists:foldl(fun(Node, State) -> emit_sn(Node, [], State, Fd) end, {[], [], maps:new()}, Mod#module.children),
...
emit_sn(Node, ParentPath, State, Fd) ->
    ...
    lists:foldl(fun(Child, NextState) -> emit_sn(Child, Path, NextState, Fd) end, NewState, Node#sn.children).

emit_sn_json(Node, PathStr, TypeDefs, Extras0, SubsFilterFn) ->
    Extras1 = Extras0 ++
    ...
    NodeJson = io_lib:format("    \"~s\": {~n", [PathStr]) ++
        io_lib:format("      \"name\": \"~s\",~n", [kw2str(Node#sn.name)]) ++
...
emit_type_spec(_, #string_type_spec{min = Min, max = Max, patterns = Patterns}, Indent) ->
    TypeName =
        case {Min, Max} of
            {0, infinity} -> "string";
    ...
    case length(Patterns) > 0 of
        true ->  emit_pattern_type(TypeName, [?b2l(Regexp) || { _, Regexp, _ } <- Patterns], Indent);
...
submode_children(Node, KeyChildren) ->
    FilterFn = fun(Child) ->
                       ((not lists:member(Child#sn.name, KeyChildren)) andalso
...
nonsubmode_children(Node, KeyChildren) ->
    FilterFn = fun(Child) ->
                       (not lists:member(Child#sn.name, KeyChildren))
               end,
    child_names(lists:filter(FilterFn, Node#sn.children)).
...
key_children(Node) ->
    PrefKeysWithIdx = lists:filter(fun({ _, Idx }) -> Idx > 0 end,
                                   lists:map(fun(Child) ->
                                                     Name = case Child#sn.kind of
                                                                'choice' -> ?l2a(lists:flatten(io_lib:format("#~s", [kw2str(Child#sn.name)])));
                                                                _ -> Child#sn.name
							    end,
